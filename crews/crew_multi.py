...
class MultiRecommendationCrew:
    """
    Crew pipeline for generating and ranking recommendations.

    Loads agent and task configs from YAML files.
    Workflow:
      1. matcher agent fetches candidates.
    ...
    def fetch_candidates_multi_task(self) -> Task:
        ...
        return Task(config=self.tasks_config["fetch_candidates_multi_task"], 
                    agent=self.matcher())
    ...
    @crew
    def multi_crew(self) -> Crew:
        """Create the full multi-activity recommendation workflow."""
        return Crew(
            ...
            agents=self.agents,
            tasks=[self.fetch_candidates_multi_task(), self.rank_and_select_task()],
